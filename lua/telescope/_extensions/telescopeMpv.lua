...
function M.toggle_player()
  ...
  map_cmd('>', '>\n')
  map_cmd('<', '<\n')
  map_cmd('+', '0\n') -- Sube el volumen (mpv usa '0')
  map_cmd('-', '9\n') -- Baja el volumen (mpv usa '9')
end
...
local function api_call(cb)
  local url = "https://youtube.googleapis.com/youtube/v3/search?part=snippet&maxResults=25&q=" .. opts.query .. "&key=" .. opts.envar
  job_lib:new({
    command = "curl",
...
local function make_picker()
  pickers.new({}, {
    prompt_title = "YouTube Search: " .. opts.query,
...
function M.single()
  opts.query = vim.fn.input("Buscar YouTube: ")
...
vim.api.nvim_create_user_command('Take', M.single, { desc = "YouTube: Buscar música" })
vim.api.nvim_create_user_command('Takep', M.playlist, { desc = "YouTube: Buscar playlist" })
